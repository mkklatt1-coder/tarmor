...
class QualityMaintenanceDocument(models.Model):
    QM_TYPE_CHOICES = [
        ('CALENDAR', 'Calendar'),
        ('METER', 'Meter'),
    ]
    ...
    single_interval_value = models.DecimalField(max_digits=12, decimal_places=0, null=True, blank=True)
    ...
    single_interval_parts_list = models.FileField(upload_to='qm_partslists/', null=True, blank=True)
    ...
    class Meta:
        ...
        ordering = ['qm_number']
    def __str__(self):
        return f'{self.qm_number} - {self.description}'
    @classmethod
...
class QualityMaintenanceDocumentStep(models.Model):
    CALENDAR_UNIT_CHOICES = [
        ('DAY', 'Days'),
        ('WEEK', 'Weeks'),
        ('MONTH', 'Months'),
    ...
    interval_value = models.DecimalField(max_digits=12, decimal_places=0)
    ...
    step_parts_list = models.FileField(upload_to='qm_partslists/', null=True, blank=True)
    class Meta:
        db_table = 'planning_qualitymaintenancedocumentstep'
        ordering = ['step_order']
        unique_together = ('document', 'step_order')
    def __str__(self):
        return f'{self.document.qm_number} - Step {self.step_order}'
    
class QualityMaintenancePlan(models.Model):
    document = models.ForeignKey(
        QualityMaintenanceDocument,
    ...
    equipment = models.ForeignKey(
        'equipment.Equipment',
        on_delete=models.CASCADE,
        related_name='quality_maintenance_plans'
    )
    start_date = models.DateField()
    meter_start = models.DecimalField(max_digits=12, decimal_places=0, null=True, blank=True)
    ...
    def __str__(self):
        return f'{self.document.qm_number} - {self.equipment}'
    
    def get_steps(self):
        return list(self.document.steps.order_by('step_order'))
    ...
    def get_current_meter(self):
        if self.document.qm_type != 'METER' or not self.meter_type:
    ...
    def get_next_due(self, current_meter=None):
        ...
        if document.qm_type == 'CALENDAR':
            if document.step_type == 'SINGLE':
                if not document.single_interval_value or not document.calendar_unit:
                    return {
                        'next_due_date': None,
                        'next_due_meter': None,
                        'step': None,
                ...
                next_due_date = self.add_calendar_interval(
                    self.start_date,
                    document.single_interval_value,
                    document.calendar_unit
                )
        ...
        if document.qm_type == 'METER':
            if current_meter is None:
                current_meter = self.get_current_meter()

            last_reading_log = MeterReading.objects.filter(
                Equipment=self.equipment, 
                Meter_Type=self.meter_type
            ).order_by('-Date', '-id').first()
            
            if last_reading_log and last_reading_log.Date:
                days_since_last_reading = (today - last_reading_log.Date).days
                if days_since_last_reading > 14:
                    warning_msg = f"Meter has not accumulated hours for {days_since_last_reading} days!"
            else:
                warning_msg = "No historical meter logs found for this unit!"

            avg_daily_usage = self.get_average_daily_usage()
            if document.step_type == 'SINGLE':
                if document.single_interval_value is None:
                    return {
                        'next_due_date': None,
                        'next_due_meter': None,
                        'step': None,
                ...
                interval = Decimal(document.single_interval_value)
                current_meter_dec = Decimal(current_meter)
                next_due_meter = ((current_meter_dec // interval) + 1) * interval
                next_due_date = None

                if not warning_msg and avg_daily_usage and avg_daily_usage > 0:
    ...
    def get_work_order_trigger_date(self, current_meter=None):
        ...
        if due['next_due_date']:
            return due['next_due_date'] - timedelta(days=self.document.work_order_lead_days)
        return None
...
class QualityMaintenanceInstance(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-created_at']
    def __str__(self):
        return f'{self.plan.document.qm_number} - {self.plan.equipment} - {self.status}'